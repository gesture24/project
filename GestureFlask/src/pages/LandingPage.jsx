import React, { useEffect, useRef, useState } from 'react';
import { motion } from 'framer-motion';
import { Play, X, Gauge, Clock, Hand, Cpu, Monitor, Building } from 'lucide-react';
import { baseUrl, camera } from '../config/config';
import axios from 'axios';
import { toast } from 'react-toastify';
import { getVideoFeedUrl, startGestureCapture } from '../config/api';



  const LandingPage = () => {
    const [gesture, setGesture] = useState('');
    const [objectId, setObjectId] = useState('');
    const [isCapturing, setIsCapturing] = useState(false);
    const [videoUrl, setVideoUrl] = useState(null);
    const [message, setMessage] = useState(""); // For "Gesture Saved" or other notifications
    const [timer, setTimer] = useState(null);
    const [cameraPermission, setCameraPermission] = useState('prompt');

  useEffect(() => {
    const fetchObjectId = () => {
      try {
        const storedObjectId = localStorage.getItem('userId');
        console.log(storedObjectId);
        if (storedObjectId) {
          setObjectId(storedObjectId);
        }
      } catch (error) {
        console.error("Error fetching objectId from localStorage:", error);
      }
    };

    fetchObjectId();
  }, []);

  const handleStartCapture = async () => {
    if (!objectId) {
      alert("Please enter a valid user ID.");
      return;
    }

    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video: true });
      setCameraPermission('granted');
      stream.getTracks().forEach(track => track.stop()); // Stop the stream immediately

      await startGestureCapture(objectId);
      setVideoUrl(getVideoFeedUrl(objectId));
      setIsCapturing(true);
    } catch (error) {
      console.error("Failed to start gesture capture:", error);
      if (error.name === 'NotAllowedError') {
        setCameraPermission('denied');
        setMessage("Camera access was blocked. Please allow camera access to use this feature.");
      } else {
        setMessage("An error occurred while trying to start the capture.");
      }
    }
  };

  const gestureImages = {
    'Hand Skeleton': {
      src: "data:image/jpeg;base64,",
      alt: 'Hand skeleton visualization showing connected joints with colored dots and lines',
      label: 'Hand Skeleton'
    },
    'Overlapped Hands': {
      src: 'https://www.motiongestures.com/assets/Uploads/MOG-Hand-Banner-square-ok.jpg',
      alt: 'Visualization of overlapped hands detection',
      label: 'Overlapped Hands'
    },
    'Thumbs Up': {
      src: 'https://ai.google.dev/static/mediapipe/images/solutions/gesture-recognizer.png',
      alt: 'Hand face and side view tracking',
      label: 'Thumbs Up'
    },
    'OK Gesture': {
      src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRwBP3kFnRtu8VkBrV4INjM-TkWlhX4tYMShg&s',
      alt: 'OK hand gesture recognition',
      label: 'OK Gesture'
    },
    'Swiping Gesture': {
      src: 'https://tse3.mm.bing.net/th?id=OIP.YIcXQq_JNiCq7w-lVH8yHQHaHa&pid=Api&P=0&h=180',
      alt: 'Swiping gesture demonstration',
      label: 'Swiping Gesture'
    },
    'Drawing Gesture': {
      src: 'data:image/jpeg;base64,',
      alt: 'Drawing gesture tracking',
      label: 'Drawing Gesture'
    },
    'Stop': {
      src: 'https://tse4.mm.bing.net/th?id=OIP.616I7d3RjHZtF6oCoyTZpQAAAA&pid=Api&P=0&h=180',
      alt: 'Drawing gesture tracking',
      label: 'Stop'
    },
    'Smile': {
      src: 'https://tse3.mm.bing.net/th?id=OIP.cS4ePQAZskt3k253MKP_cwHaE8&pid=Api&P=0&h=180',
      alt: 'Drawing gesture tracking',
      label: 'Smile'
    },
    'peace': {
      src: 'https://tse1.mm.bing.net/th?id=OIP.4DTXnK2J0uaFbcZ2Hq-HWwAAAA&pid=Api&P=0&h=180',
      alt: 'Drawing gesture tracking',
      label: 'Peace'
    },
    'Thumbs down': {
      src: 'https://images.pexels.com/photos/9017561/pexels-photo-9017561.jpeg?auto=compress&cs=tinysrgb&w=400',
      alt: 'Drawing gesture tracking',
      label: 'Thumbs down'
    }
  };

  const [showModal, setShowModal] = useState(false);
  const [selectedGesture, setSelectedGesture] = useState(Object.keys(gestureImages)[0]);




  const features = [
    {
      left: [
        "Utilizes 21 joints-based hand skeleton approach",
        "Supports any static, dynamic, or drawing gesture",
        "Compatible with any type of camera, in any position",
        "Deployable on ordinary, low-cost processors"
      ],
      right: [
        "Enables gesture creation without data collection",
        "Supports depth inference and hidden joints detection",
        "Integrable with joints-skeleton based body tracking",
        "Applicable to any product or app, in any industry"
      ]
    }
  ];

  return (
    <div className="min-h-screen bg-white">

      <div className="max-w-6xl mx-auto px-4 py-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">

          <div className="space-y-6">
            <motion.h1
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="text-4xl md:text-5xl font-bold text-teal-800 leading-tight"
            >
              Enhance your products with an advanced touch-free interface!
            </motion.h1>

            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }}
              className="text-xl md:text-2xl text-teal-700 leading-relaxed"
            >
              Explore cutting-edge hand tracking and gesture recognition software powered by industry-leading camera technology.
            </motion.p>

            {/* <motion.button
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.4 }}
              className="bg-teal-600 text-white px-8 py-3 rounded-full text-lg font-medium
                         hover:bg-teal-700 transform hover:scale-105 transition-all duration-300
                         hover:shadow-lg"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              Try Now
            </motion.button> */}
          </div>


          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.6 }}
            className="relative"
          >
            <img
              src="https://www.motiongestures.com/assets/Uploads/MOG-Hand-Banner-square-ok.jpg"
              alt="Hand tracking visualization"
              className="w-full max-w-md mx-auto"
            />
          </motion.div>
        </div>

        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.8 }}
          className="mt-16 bg-white rounded-full shadow-lg p-8 flex flex-col md:flex-row items-center justify-between"
        >
          <div className="flex items-center space-x-4 mb-4 md:mb-0">
            <div className="w-12 h-12">
              <img
                src="https://cdn-icons-png.flaticon.com/128/12350/12350545.png"
                alt="Logo"
                className="w-full h-full object-contain"
              />
            </div>
            <div>
              <h2 className="text-teal-800 text-xl font-bold">
                See the robustness of our hand tracking and gesture recognition software!
              </h2>
              <p className="text-teal-600">Try it now!</p>
            </div>
          </div>

          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => setShowModal(true)}
            className="bg-teal-600 text-white px-6 py-3 rounded-full flex items-center space-x-2
                       hover:bg-teal-700 transform transition-all duration-300 hover:shadow-lg"
          >
            <Play size={20} />
            <span>Try Now</span>
          </motion.button>
        </motion.div>


        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
          className="mt-24 max-w-5xl mx-auto"
        >
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-4xl font-bold text-teal-800 mb-6 text-center"
          >
            The most advanced camera software worldwide, adaptable for any platform and application
          </motion.h2>

          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="text-lg text-[#8B7355] text-center mb-12"
          >
            Provides exceptional hand tracking and gesture recognition accuracy powered by groundbreaking AI algorithms.
          </motion.p>

          <div className="grid md:grid-cols-2 gap-x-16 gap-y-6">
            <motion.ul
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.4 }}
              className="space-y-4"
            >
              {features[0].left.map((feature, index) => (
                <motion.li
                  key={index}
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.4 + index * 0.1 }}
                  className="flex items-center text-[#8B7355] text-lg"
                >
                  <span className="w-2 h-2 bg-teal-600 rounded-full mr-3"></span>
                  {feature}
                </motion.li>
              ))}
            </motion.ul>


            <motion.ul
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.4 }}
              className="space-y-4"
            >
              {features[0].right.map((feature, index) => (
                <motion.li
                  key={index}
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.4 + index * 0.1 }}
                  className="flex items-center text-[#8B7355] text-lg"
                >
                  <span className="w-2 h-2 bg-teal-600 rounded-full mr-3"></span>
                  {feature}
                </motion.li>
              ))}
            </motion.ul>
          </div>
        </motion.div>


        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1.2 }}
          className="mt-24 max-w-5xl mx-auto"
        >
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-4xl font-bold text-teal-800 mb-6"
          >
            Explore hand tracking and gesture recognition photos
          </motion.h2>

          <div className="grid md:grid-cols-2 gap-12 items-start">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="relative bg-white rounded-lg shadow-lg overflow-hidden"
              key={selectedGesture}
            >
              <motion.img
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.3 }}
                src={gestureImages[selectedGesture].src}
                alt={gestureImages[selectedGesture].alt}
                className="w-full object-cover"
              />
              <div className="absolute bottom-0 left-0 right-0 bg-teal-800 text-white py-4 px-6">
                <h3 className="text-xl font-semibold">{gestureImages[selectedGesture].label}</h3>
              </div>
            </motion.div>


            <div className="space-y-4">
              <h3 className="text-2xl text-teal-800 font-semibold mb-6">
                Select from following gestures:
              </h3>
              <div className="space-y-3">
                {Object.keys(gestureImages).map((gesture) => (
                  <motion.button
                    key={gesture}
                    onClick={() => setSelectedGesture(gesture)}
                    className={`w-full text-left px-6 py-4 rounded-lg transition-all duration-300 
                              ${selectedGesture === gesture
                        ? 'bg-teal-600 text-white shadow-md'
                        : 'bg-gray-100 text-gray-700 hover:bg-teal-50'}`}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    {gesture}
                  </motion.button>
                ))}
              </div>

            </div>
          </div>
        </motion.div>
      </div>

      <div className="max-w-7xl mx-auto px-4 py-16 p-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <motion.div
            className="lg:col-span-4"
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6 }}
          >
            <h1 className="text-4xl lg:text-4xl font-bold text-[#0B3B5B] leading-tight">
              Our camera-based software will revolutionize your user experience
            </h1>
          </motion.div>


          <motion.div
            className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-6"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.2 }}
          >

            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 mb-4 rounded-full border-2 border-teal-500  flex items-center justify-center p-4">
                <img
                  src="https://www.motiongestures.com/assets/Uploads/HomepageIcons_100Percent.png"
                  alt="Accuracy icon"
                  className="w-full h-full object-contain"
                />
              </div>
              <p className="text-lg font-medium text-teal-800">~100% hand tracking and gesture recognition accuracy</p>
            </div>


            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 mb-4 rounded-full border-2 border-teal-500  flex items-center justify-center p-4">
                <img
                  src="https://www.motiongestures.com/assets/Uploads/HomepageIcons_-ReduceTime.png"
                  alt="Time reduction icon"
                  className="w-full h-full object-contain"
                />
              </div>
              <p className="text-lg font-medium text-teal-800">10x reduction in development cost & time to market</p>
            </div>


            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 mb-4 rounded-full border-2 border-teal-500 flex items-center justify-center p-4">
                <img
                  src="https://www.motiongestures.com/assets/Uploads/HomepageIcons_Gestures.png"
                  alt="Gesture support icon"
                  className="w-full h-full object-contain"
                />
              </div>
              <p className="text-lg font-medium text-teal-800">Supports any static, dynamic, or drawing gesture</p>
            </div>


            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 mb-4 rounded-full border-2 border-teal-500 flex items-center justify-center p-4">
                <img
                  src="https://www.motiongestures.com/assets/Uploads/HomepageIcons_AI.png"
                  alt="Deployment icon"
                  className="w-full h-full object-contain"
                />
              </div>
              <p className="text-lg font-medium text-teal-800">
                Optimized for embedded deployment
                <span className="block text-sm text-teal-600 mt-1">(AI at the Edge)</span>
              </p>
            </div>


            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 mb-4 rounded-full border-2 border-teal-500 flex items-center justify-center p-4">
                <img
                  src="https://www.motiongestures.com/assets/Uploads/HomepageIcons_PlatformAgnostic.png"
                  alt="Platform icon"
                  className="w-full h-full object-contain"
                />
              </div>
              <p className="text-lg font-medium text-teal-800">
                Platform agnostic
                <span className="block text-sm text-teal-600 mt-1">(any camera, processor, or OS)</span>
              </p>
            </div>


            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 mb-4 rounded-full border-2 border-teal-500 flex items-center justify-center p-4">
                <img
                  src="https://www.motiongestures.com/assets/Uploads/HomepageIcons_ApplictionAgnostic.png"
                  alt="Application icon"
                  className="w-full h-full object-contain"
                />
              </div>
              <p className="text-lg font-medium text-teal-800">Application agnostic: any use-case in any industry</p>
            </div>
          </motion.div>
        </div>
      </div>


      {showModal && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-6"
        >
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-white rounded-xl p-6 w-[110vw] max-w-5xl relative"
          >
            <button
              onClick={() => setShowModal(false)}
              className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 transition-colors"
            >
              <X className="h-6 w-6" />
            </button>

            <div className="bg-gray-100 rounded-lg p-6 min-h-[60vh] aspect-auto flex-1 flex">
              <div className="w-full h-full flex flex-col items-center justify-center text-gray-500">
                {!isCapturing ? (
                  <button
                    onClick={handleStartCapture}
                    className="px-6 py-3 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors font-medium"
                  >
                    Start Gesture Capture
                  </button>
                ) : (
                  videoUrl && (
                    <div className="w-full flex flex-col items-center gap-4 flex-1">
                      <h2 className="text-xl font-semibold text-gray-900">
                        Gesture Capture Live Feed
                      </h2>
                      <img
                        src={videoUrl}
                        alt="Gesture Live Feed"
                        className="w-full- h-full object-cover rounded-lg"
                      />
                    </div>
                  )
                )}
                {message && (
                  <p className="mt-4 text-red-500">{message}</p>
                )}
                {cameraPermission === 'denied' && (
                  <p className="mt-4 text-red-500">
                    Camera access is blocked. Please allow camera access in your browser settings to use this feature.
                  </p>
                )}
              </div>
            </div>
          </motion.div>
        </motion.div>
      )}
    </div>
  );
};

export default LandingPage;